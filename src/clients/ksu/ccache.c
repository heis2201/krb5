 ...
 */
krb5_error_code
krb5_ccache_copy(context, cc_def, target_principal, cc_target, restrict_creds,
                 primary_principal, stored)
    /* IN */
...
krb5_error_code
krb5_store_all_creds(context, cc, creds_def, creds_other)
    krb5_context context;
    krb5_ccache cc;
...
krb5_boolean
compare_creds(context, cred1, cred2)
    krb5_context context;
    krb5_creds *cred1;
    krb5_creds *cred2;
{
...
krb5_error_code
krb5_get_nonexp_tkts(context, cc, creds_array)
    krb5_context context;
    krb5_ccache cc;
    ...
    while (!(retval = krb5_cc_next_cred(context, cc, &cur, &creds))) {
        ...
        if (!krb5_is_config_principal(context, creds.server) &&
            ...
            if (count == (chunk_count * CHUNK -1)) {
                chunk_count++;
                temp_creds = (krb5_creds **)realloc(temp_creds, chunk_count *
                                                    CHUNK *
                                                    sizeof(krb5_creds *));
                if (!temp_creds)
                    return ENOMEM;
...
krb5_error_code
krb5_check_exp(context, tkt_time)
    krb5_context context;
    ...
    time_t tv;
{
    ...
    /* ASSUMES sizeof(krb5_timestamp) >= sizeof(time_t) */
...
krb5_error_code
krb5_store_some_creds(context, cc, creds_def, creds_other, prst, stored)
    krb5_context context;
    krb5_ccache cc;
    krb5_creds **creds_def;
    krb5_creds **creds_other;
...
krb5_error_code
krb5_ccache_filter(context, cc, prst)
    krb5_context context;
    krb5_ccache cc;
    krb5_principal prst;
{
...
krb5_boolean
krb5_find_princ_in_cred_list(context, creds_list, princ)
    krb5_context context;
...
krb5_error_code
krb5_find_princ_in_cache(context, cc, princ, found)
    krb5_context context;
    krb5_ccache cc;
    krb5_principal princ;
    krb5_boolean *found;
