...
/*
 * get_all_princ_from_file - retrieves all principal names from file pointed
 * to by fp.
 */
...
krb5_error_code
get_all_princ_from_file (fp, plist)
    FILE *fp;
    char ***plist;
{
...
/*
 * list_union - combines list1 and list2 into combined_list. The  space for
 * list1 and list2 is either freed or used by combined_list.
 */
krb5_error_code
list_union(list1, list2, combined_list)
    char **list1;
    char **list2;
    char ***combined_list;
{
    ...
    char ***k5users_filt_list;
{
    ...
    *k5users_filt_list = NULL;
    ...
    temp_filt_list = (char **)calloc(found_count + 1, sizeof(char*));
    if (!temp_filt_list)
        return ENOMEM;
 ...
 * himself). plist contains a principal list obtained from .k5login and
 * .k5users file. A principal is picked that has the best chance of getting in.
 */
krb5_error_code
get_closest_principal(context, plist, client, found)
 ...
 */
krb5_error_code
find_either_ticket (context, cc, client, end_server, found)
    krb5_context context;
    krb5_ccache cc;
    krb5_principal client;
...
krb5_error_code
find_ticket (context, cc, client, server, found)
    krb5_context context;
    krb5_ccache cc;
    krb5_principal client;
...
krb5_error_code
find_princ_in_list(context, princ, plist, found)
    krb5_context context;
...
/*
 * get_best_princ_for_target - sets the client name, path_out gets set, if
 * authorization is not possible path_out gets set to ...
 */
krb5_error_code
get_best_princ_for_target(context, source_uid, target_uid, source_user,
                          target_user, cc_source, options, cmd, hostname,
                          client, path_out)
    krb5_context context;
    uid_t source_uid;
    uid_t target_uid;
    ...
    int *path_out;
{
    ...
    *path_out = 0;
    ...
    if (source_uid == 0){
        if (target_uid != 0) {
            /* This will be used to restrict the cache copty */
    ...
    /*
     * Girst see if default principal of the source cache
    ...
    if (auth_debug) {
        printf("GET_best_princ_for_target: out of luck, can't get "
               "appropriate default principal\n");
    }
    *path_out = NOT_AUTHORIZED;
    return 0;
}
