...
#define TKTTIMELEFT 60 * 10   /* Ten (10) minutes */
...
static void
get_init_creds_opt_init(krb5_get_init_creds_opt *outOptions)
{
...
static kbrccache_t
userinitcontext(const char * user, const char * domain, const char * passwd,
                const char * cachename, int initialize, int * outError)
{
    ...
    if (domain) {
        kres = krb5_build_principal(kcontext, &kme, strlen(domain), domain,
                                    user, (char *) 0);
    } else {
    ...
    if (kres == 0 && user && passwd) {
        ...
        numCreds = 0;
        kres = krb5_cc_start_seq_get(kcontext, kcache, &cc_curs);
        if (kres >= 0) {
            kres = krb5_cc_next_cred(kcontext, kcache, &cc_curs, &kcreds);
            while (kres == 0) {
                numCreds++;
                if (krb5_principal_compare(kcontext, kme, kcreds.client)) {
                    if (kcreds.ticket_flags & TKT_FLG_INITIAL &&
                        kcreds.times.endtime > timeneeded)
                        have_credentials = 1;
                }
        ...
        if (kres != 0 || have_credentials == 0) {
            ...
            if (kres == 0) {
                ...
                kres = krb5_get_init_creds_password(kcontext, &kcreds, kme,
                                                    pPass,
                                                    NULL, /* prompter */
                                                    NULL, /* data */
                                                    0, /* starttime */
    ...
    for (;;) {
        ...
        }
        {
            ...
            if (!password)
                return -1;
            result = 0;
            usercontext = userinitcontext(user, NULL, password, NULL, 1,
                                          &result);
            if (usercontext)
                break;
        }
...
int main(int argc, char ** argv)
{
    ...
    /* Change the password */
    {
        ...
        if (kerr == 0) {
            kerr = krb5_set_password_using_ccache(kcontext, ccache,
                                                  new_password,
                                                  target_principal,
                                                  &pw_result, &pw_res_string,
                                                  &res_string);
            if (kerr) {
